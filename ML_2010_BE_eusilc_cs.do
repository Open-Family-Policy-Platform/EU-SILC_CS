...
replace ml_eli = 1 		if country == "BE" & year == 2010 & gender == 1 ///
						& inrange(econ_status,1,3) & (duremp+dursemp) >= 6
replace ml_eli = 0 		if ml_eli == . & country == "BE" & year == 2010 & gender == 1
...
replace ml_dur1 = 1 	if country == "BE" & year == 2010 & gender == 1 ///
						& ml_eli == 1



			
replace ml_dur2 = 15-1 	if country == "BE" & year == 2010 & gender == 1 ///
						& ml_eli == 1
			
replace ml_dur2 = 8-1 	if country == "BE" & year == 2010 & gender == 1 ///
...
* BENEFIT (monthly)
/* 	
	-> public sector: 100% earning (LP&R 2010)
	-> private sector:  
		-> first 30 days = 82% earnings, no ceiling 
		-> rest of leave = 75% earnings, ceiling €88.77/day (LP&R 2010)
		
	-> unemployed: no information available in MISSOC or LP&R 
	
		
	-> self-employed (MISSOC self-employed, 1/7/2010):
		-> €375.72/week
*/
...
replace ml_ben1 = (((((0.82*earning) / 4.3) * (30/5)) + (((0.75*earning)/4.3) * (15 - (30/5)))) / 15) * 4.3 /// 
						if country == "BE" & year == 2010 & gender == 1 ///
...
* self-employed
replace ml_ben1 = 375.72 * 4.3 		if country == "BE" & year == 2010 ///
...
foreach x in 1 2 {
    replace ml_dur`x' = 0 	if ml_eli == 0 & country == "BE" & year == 2010
	replace ml_ben`x' = 0 	if ml_eli == 0 & country == "BE" & year == 2010
}
