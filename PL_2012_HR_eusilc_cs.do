...
replace pl_dur = 3*4.3 		if country == "HR" & year == 2012 & pl_eli == 1 ///
							& inlist(econ_status,1,2) & (duremp+dursemp) >= 12 ///
							& childc <= 2
						
replace pl_dur = 15/4.3 		if country == "HR" & year == 2012 & pl_eli == 1 ///
								& inlist(econ_status,1,2) & (duremp+dursemp) >= 12 ///
								& childc > 2 
						
replace pl_dur = 6*4.3 		if country == "HR" & year == 2012 & pl_eli == 1 ///
							& pl_dur == . & childc <= 2 & gender == 1
						
replace pl_dur = (3*52) - ml_dur2 		if country == "HR" & year == 2012 & pl_eli == 1 ///
										& pl_dur == . & childc > 2 & gender == 1
...
/*	-> Employed & self-employd: 
			-> 100%
			-> ceiling: 80% of the budgetary base rate (€440/month)
	-> non-working: 
...
replace pl_ben1 = earning  		if country == "HR" & year == 2012 & pl_eli == 1 ///
	...
	* ceiling
replace pl_ben1 = 0.8*440 		if country == "HR" & year == 2012 & pl_eli == 1 ///
								& inlist(econ_status,1,2) & (duremp+dursemp) >= 12 ///
								& pl_ben1 > (0.8*440)
								
* non-working
replace pl_ben1 = 0.5*440	 	if country == "HR" & year == 2012 & pl_eli == 1 ///
								& pl_ben1 == . 
								

replace pl_ben2 = pl_ben1 		if country == "HR" & year == 2012 & pl_eli == 1
