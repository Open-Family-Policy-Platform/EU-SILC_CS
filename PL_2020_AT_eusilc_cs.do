...
*/
replace pl_eli = 1 	if country == "AT" & year == 2020 
replace pl_eli = 0 	if pl_eli == . & country == "AT" & year == 2020
...
* employed   
	* eligible for ML
	* and cohabiting
replace pl_dur = (365/7) - ml_dur2 	if country == "AT" & year == 2020 & pl_eli == 1 ///
...
replace pl_dur = (365/7) - ml_dur2 	if country == "AT" & year == 2020 & pl_eli == 1 ///
	...
	* not eligible for ML
	* and cohabiting
replace pl_dur = (365/7) 	if country == "AT" & year == 2020 & pl_eli == 1 ///
							& gender == 1 & (econ_status == 1 | p_econ_status == 1) ///
...
/*	-> benefit is a family entitlement but among working parents it calculated 
	...
	-> NOTE: only the income of the parent who receives the benefit is considered! (MISSOC 2020)
*/


  ** employed 
replace pl_ben1 = 0.8 * earning 	if country == "AT" & year == 2020 & pl_eli == 1 /// 
									& econ_status == 1 
									
									
replace pl_ben1 = 66 * 21.7			if country == "AT" & year == 2020 & pl_eli == 1 /// 
									& econ_status == 1 & pl_ben1 >= (66*21.7)								
 
 ** not employed 
replace pl_ben1 = 33.88 * 21.7	 	if country == "AT" & year == 2020 & pl_eli == 1 /// 
									& inrange(econ_status,2,4) 
 
 
replace pl_ben2 = pl_ben1		if country == "AT" & year == 2020 & pl_eli == 1

									
replace pl_ben2 = pl_ben1		if country == "AT" & year == 2020 & pl_eli == 1									
									
foreach x in 1 2 {
	replace pl_ben`x' = 0 	if country == "AT" & year == 2020 & pl_ben`x' == .
}

replace pl_dur = 0 	if country == "AT" & year == 2020 & pl_dur == . 
