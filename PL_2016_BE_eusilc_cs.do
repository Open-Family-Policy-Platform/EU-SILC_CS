...
/*	-> private sector: employees, 12 months (coded) of employement in last 15 months with the same employer (not 
		coded)
	-> public sector: employees (not coded; LP&R 2016)						*/
   
replace pl_eli = 1 		if country == "BE" & year == 2016 & pl_eli == . ///
						& econ_status == 1 & duremp >= 12 
replace pl_eli = 0 		if pl_eli == . & country == "BE" & year == 2016
...
foreach x in 1 2 {
	replace pl_ben`x' = 0 	if pl_eli == 0 & country == "BE" & year == 2016
}

replace pl_dur = 0 	if pl_eli == 0 & country == "BE" & year == 2016
