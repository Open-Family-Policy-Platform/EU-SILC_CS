...
replace ml_eli = 1 			if country == "PT" & year == 2014 & gender == 1 ///
							& inlist(econ_status,1,2) & (duremp + dursemp) >= 6

							
replace ml_eli = 0 			if ml_eli == . & country == "PT" & year == 2014 & gender == 1


* DURATION (weeks)
/*	-> postnatal leave: 6 weeks
	-> prenatal leave: no compulsory leave, option of 30 days from the leave (not coded) */
replace ml_dur1 = 0 		if country == "PT" & year == 2014 & ml_eli == 1

replace ml_dur2 = 6 		if country == "PT" & year == 2014 & ml_eli == 1
...
replace ml_ben1 = earning 		if country == "PT" & year == 2014 & ml_eli == 1
replace ml_ben1 = 419.22		if country == "PT" & year == 2014 & ml_eli == 1 ///
								& ml_ben1 < 419.22
								
replace ml_ben2 = ml_ben1 		if country == "PT" & year == 2014 & ml_eli == 1

foreach x in 1 2 {
    replace ml_dur`x' = 0 	if ml_eli == 0 & country == "PT" & year == 2014
	replace ml_ben`x' = 0 	if ml_eli == 0 & country == "PT" & year == 2014
}
