...
replace pt_eli = 1 		if country == "PT" & year == 2010 & gender == 2 ///
						& inlist(econ_status,1,2) & (duremp+dursemp) >= 6

					
						
replace pt_eli = 0 		if pt_eli == . & country == "PT" & year == 2010 & gender == 2

* DURATION (weeks)
/*	-> 20 working days (LP&R 2010), 10 days are obligatory, within the first month after birth
	-> LP&R 2010: single parents cannot use the other parent's entitlement => 
		father's share is not assigned to single woman 	*/

replace pt_dur = 20/5 	if country == "PT" & year == 2010 & pt_eli == 1
...
replace pt_ben1 = earning  	if country == "PT" & year == 2010 & pt_eli == 1

replace pt_ben1 = 419.22  	if country == "PT" & year == 2010 & pt_eli == 1 ///
							& pt_ben1 < 419.22

replace pt_ben2 = pt_ben1 	if country == "PT" & year == 2010 & pt_eli == 1


foreach x in 1 2 {
	replace pt_ben`x' = 0 	if pt_eli == 0 & country == "PT" & year == 2010
}

replace pt_dur = 0 if pt_eli == 0 & country == "PT" & year == 2010
